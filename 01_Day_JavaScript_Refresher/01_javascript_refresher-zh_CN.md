...
- [JavaScript复习](#JavaScript复习)
  - [0. 将JavaScript加入网页](#0.-将JavaScript加入网页)
    - [单行脚本](#单行脚本)
    - [内部脚本](#内部脚本)
    - [外部脚本](#外部脚本)
    - [多个外部脚本](#多个外部脚本)
  ...
  - [3. 数组](#3.-数组)
    - [如何创建一个空数组](#如何创建一个空数组)
    - [如何创建一个有值的数组](#如何创建一个有值的数组)
    - [使用split函数创建数组](#使用split函数创建数组)
    - [使用index下标获取数组元素](#使用index下标获取数组元素)
    - [修改数组元素](#修改数组元素)
    - [操作数组的方法](#操作数组的方法)
      - [数组构造器](#数组构造器)
...
在桌面或者任何地方创建一个名为30DasyOfJS 的项目,并在目录中创建**_index.html_**文件.将以下内容复制到文件中,并使用Chrome浏览器打开
...
现在，你创建了第一个单行脚本.我们能够通过_alert()_函数创建要给弹窗警示信息.


#### 内部脚本

内部脚本可以写在_head_也可以写在_body_，在写HTML文档时，建议写在_body_中.
...
```
我们大多数时候按照上面的方法写内部脚本.我们更倾向选择在body中写JavaScript代码。打开浏览器控制台，我们能看到`console.log()`输出的内容。
...
打开浏览器控制台，我们能看到`console.log()`输出的内容。

![js code from vscode](../images/js_code_vscode.png)

#### 外部脚本

外部脚本链接可以放在head或body中.但是更推荐放在body中.

首先我们创建一个带有.js扩展名的JavaScript文件.所有代码.js扩展名的文件都是JavaScript脚本.在项目中创建一个名为**_intruduction.js_**的文件目录，然后将以下代码，并将.js文件链接写到body的底部。
...
```
打开浏览器的控制台查看 console.log() 输出的内容

#### 多个外部脚本

我们可以在网页中引入多个JavaScript文件.在30DaysOfJS目录中创建一个名为 helloworld.js 的文件并写入下面代码
...
```
有一点非常重要：_你的main.js文件应该包含所有其它的脚本_.

![Multiple Script](../images/multiple_script.png)

### 1. 变量
我们使用 _var_, _let_ 和 _const_定义变量._var_是函数作用域, 然后 _let_ 和 _const_ 是块作用域. 在这个挑战中，我们使用ES6版本的JavaScript。所以不使用 _var_
...
如果你觉得不适应数据类型，可以查看[此链接](https://github.com/Asabeneh/30-Days-Of-JavaScript/blob/master/02_Day_Data_types/02_day_data_types.md)

### 3. 数组

与变量相比，数组可以存储 _多个值_.每个在数组中值都有一个 _index_，用来标记_在内存中的地址_。每个值能够通过 _index_被访问. _index_ 从 _0_ 开始，最后一位比数组长度小1.

数组可以存储不同类型的的数据。

#### 如何创建一个空数组

在JavaScript中，我们有多种方式创建一个数组.让我们看看有那些方式创建数组.
一般我们使用 _const_ 而不是 _let_ 定义一个数组变量.如果使用了 _const_ 就意味着不会多次定义同一个变量名.
...
创建一个有初始值的数组. 我们使用 _length_ 属性获取数组的长度
...
有多种不通的方法操作数组，比如: _Array, length, concat, indexOf, slice, splice, join, toString, includes, lastIndexOf, isArray, fill, push, pop, shift, unshift_
